...
export function buildProjectDocumentHtml(
  ...
  const amt =
    project.contractAmount != null && project.contractAmount > 0
      ? formatINR(project.contractAmount)
      : "—";
...
export function createGeneratedDocumentRow(
  ...
  const { title, bodyHtml } = buildProjectDocumentHtml(docKey, project, quotation);
  return {
    id,
    docKey,
    title,
