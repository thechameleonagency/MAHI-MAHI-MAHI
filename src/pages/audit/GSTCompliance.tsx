        ...
        subRow={
          <>
            ...
            <InlineKpiStrip
              ...
              items={[
                { label: "Output GST", value: fmt(gstSummary.outputGST) },
                { label: "Input GST", value: fmt(gstSummary.inputGST) },
                { label: "Net payable", value: fmt(gstSummary.netPayable) },
              ]}
            />
          </>
        }
                  ...
                  <div className="space-y-2">
                    {gstr1.b2b.map(r => (
                      <div key={r.id} className="flex justify-between items-center p-2 rounded border border-border">
                        ...
                        <span className="text-sm font-medium">{fmt(r.total)}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <CardContent>
              <div className="space-y-4 max-w-lg">
                <div className="flex justify-between py-3 border-b border-border">
                  ...
                  <span className="text-sm font-bold">{fmt(salesRegister.reduce((s, r) => s + r.taxableValue, 0))}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-border">
                  <span className="text-sm font-medium text-foreground">3.1 Output Tax (CGST + SGST + IGST)</span>
                  <span className="text-sm font-bold text-primary">{fmt(gstSummary.outputGST)}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-border">
                  <span className="text-sm font-medium text-foreground">4. Input Tax Credit</span>
                  <span className="text-sm font-bold text-blue-600">{fmt(gstSummary.inputGST)}</span>
                </div>
                <div className="flex justify-between py-4 bg-muted/50 px-3 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-orange-600">{fmt(gstSummary.netPayable)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
